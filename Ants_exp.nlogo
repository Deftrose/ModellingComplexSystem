...
to setup-nest  ;; patch procedure
  ...
  set nest? (distancexy -10 0) < 5
  ;; spread a nest-scent over the whole world -- stronger near the nest
  set nest-scent 200 - distancexy -10 0
end

to setup-food  ;; patch procedure
  ;; setup food source one on the lower-right
  if (distancexy (source_a_dist * max-pxcor) (- source_a_dist * max-pycor)) < 5
  [ set food-source-number 1 ]
  ;; setup food source three on the upper-right
  if (distancexy (source_b_dist * max-pxcor) (source_b_dist * max-pycor)) < 5
...
to recolor-patch  ;; patch procedure
  ...
  [ ifelse food > 0
    [ if food-source-number = 1 [ set pcolor green ]
      if food-source-number = 2 [ set pcolor blue  ] ]
    ;; scale color to show chemical concentration
    [ set pcolor scale-color violet chemical 0.1 5 ] ]
end
...
to go  ;; forever button
  ...
  or sum [food] of patches with [pcolor = blue] = 0 [ stop ]
...
"" ""
PENS
"food-in-pile2" 1.0 0 -13840069 true "" "plotxy ticks sum [food] of patches with [pcolor = green]"
