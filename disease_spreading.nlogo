...
[
  ...
  recovered?          ;; If true, the person has recovered from an infection;
                      ;;   they cannot be re-infected.
  infected-days       ;; Remaining number of days infected (for v2)
  recovered-days      ;; Remaining number of days recovered (for v2)
]
...
to setup-people
  ...
  [
    ...
    ifelse initial-dispersed?
    [
      ;; set 2% of individuals to be infected at random
      if random 100 < 2 [ set infected? true ]
    ]
    [
      ;; set initial infected as a circle of radius 10
      if (distancexy 0 0) < 10 [ set infected? true ]
    ]
...
;; Different people are displayed in 3 different colors depending on health status
;;   White is a susceptible person
;;   Green is a recovered person
...
to assign-pcolor  
  ...
  if recovered?
    [ set pcolor green ]
  if not infected? and not recovered?
    [ set pcolor white ]
end
...
to go
  ;; run for 500 ticks or until there are no more infected people
  if ticks = 500 [ stop ]
  if count patches with [ infected? ] = 0 [ stop ] 
  
  ;; ask those cells which are infected to infect other cells;
  ;; also check if they recover
  ;;
  ;; (v2 NOTE: you will need to modify and add to the following lines!)
  ;;
...
to check-infect  
   ;; Infection can occur to any susceptible person nearby an infected person
   let nearby-susceptible neighbors with [ not infected? and not recovered? ]

   ask nearby-susceptible
     [
       ;; If cell is in state Susceptible and neighboring cell is in state
       ;; Infected, then spread infection with probability infect-chance.
...
to check-recover
  ;; If cell is in state Infected, then recover from disease with 
  ;; probability recover-chance.
...
"" ""
PENS
"Infected" 1.0 0 -2674135 true "" "plot count patches with [ infected? ]"
"Recovered" 1.0 0 -10899396 true "" "plot count patches with [ recovered? ]"
"Susceptible" 1.0 0 -16777216 true "" "plot count patches - count patches with [recovered?] - count patches with [infected?]"
...
"" ""
PENS
"% infected" 1.0 0 -2674135 true "" "plot (((count patches with [ recovered? ] + count patches with [ infected? ]) / count patches) * 100)"
"% recovered" 1.0 0 -10899396 true "" "plot ((count patches with [ recovered? ] / count patches) * 100)"
...
This model incorporates all of the above assumptions and shows the disease spread as a phenomenon with an element of stochasticity. Small perturbations in the parameters included here can in fact lead to different final outcomes.
@#$#@#$#@
default
true
0
